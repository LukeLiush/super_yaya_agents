...
class GetNewsArgs(BaseModel):
    ...
    num_stories: int = Field(
        description="Number of stories to request. Note: The underlying source (yfinance) is typically capped at 10 items.",
        default=10,
...
def yfinance_get_company_news(ctx: RunContext[YFinanceTools], get_news_args: GetNewsArgs) -> str:
    ...
    try:
        ...
        logger.error(f"Error fetching news for {get_news_args.ticker}: {str(e)}")
        raise
...
async def build_fetch_news_agent() -> PrefectAgent:
    ...
    agent = Agent(name="Finance Agent",
                  ...
                  system_prompt=(
                      ...
                      "Your task is to fetch the latest news articles related to a given stock ticker and analyze their sentiments. "))
...
async def fetch_asset_news(ticker: str) -> News:
    ...
    yfinance_tools = YFinanceTools(enable_company_news=False,
                                   enable_company_info=False,
                                   enable_historical_prices=False,
                                   enable_stock_price=True)
    prefect_agent: PrefectAgent = await build_fetch_news_agent()
    result = \
        await prefect_agent.run(f"Get the latest news articles for the stock ticker [{ticker}] "
                                f"and analyze {num_stories} stories.", deps=yfinance_tools)
