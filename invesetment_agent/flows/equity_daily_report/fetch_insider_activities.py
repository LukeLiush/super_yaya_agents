...
class GetInsiderActivityArg(BaseModel):
    ticker: str = Field(
        description="The stock ticker symbol (e.g., 'AAPL'). Must be uppercase.",
        pattern=r"^[A-Z]{1,5}$"
    )
    recent_days: int = Field(
        description="Number of days to look back for insider trading activities.",
        default=90,
        ge=1,
        le=365
    )
...
class InsiderTransaction(BaseModel):
    ...
    code: str = Field(
        alias="Code",
        description="SEC transaction code (e.g., 'P' for open market purchase, 'S' for open market sale, 'M' for exercise of derivatives, 'G' for gift)."
    )
    description: str = Field(
        alias="Description",
        description="A human-readable explanation of the transaction code (e.g., 'Open Market Sale' or 'Grant, award or other acquisition')."
    )
    shares: float = Field(
        alias="Shares",
        description="The number of shares involved in the specific transaction."
    )
    ...
    value: Optional[float] = Field(
        alias="Value",
        description="Total monetary value of the transaction (Shares * Price)."
    )
    ...
    form: str = Field(
        alias="Form",
        description="The SEC form type, typically 'Form 4' for changes in ownership."
    )
    issuer: str = Field(
        alias="Issuer",
        description="The full legal name of the company issuing the stock (e.g., 'Tesla, Inc.')."
    )
    insider: str = Field(
        alias="Insider",
        description="The name of the individual or entity performing the trade (e.g., 'Elon Musk')."
    )
    position: str = Field(
        alias="Position",
        description="The job title or relationship of the insider to the company (e.g., 'Chief Executive Officer', 'Director', or '10% Owner')."
    )
...
def get_insider_trading_activities(get_insider_activity_arg: GetInsiderActivityArg) -> str:
    ...
    Returns:
        ...
        details such as transaction type, code, description, shares, price, value, date, form type, issuer, insider name, position, and remaining shares owned. If no filings are found
    """
    ...
    try:
        ...
        if not filings:
            logger.warning("No Form 4 filings discovered for %s in the specified timeframe",
                           get_insider_activity_arg.ticker)
            return empty_result
        ...
        logger.error("Critical failure pulling SEC records for %s: %s", get_insider_activity_arg.ticker, e,
                     exc_info=True)
        return empty_result
...
def _build_agent() -> PrefectAgent:
    ...
    agent = Agent(name="Finance Agent", model=model,
                  output_type=InsiderTradingActivities,
                  tools=[get_insider_trading_activities],
                  system_prompt=(
                      "You are an expert senior financial analyst specializing in corporate governance and SEC data analysis.\n\n"
                      ...
                      "   - Standardize titles: Use 'CEO' instead of 'Chief Executive Officer', 'CFO' for 'Chief Financial Officer', etc.\n"
                      ...
                      "   - Format: • [Role Emoji] *[Name]* ([Title]): [Signal Emoji] [Type] [Shares] shares at $[Price] on [Date].\n"
                      "   - No Data: If the tool returns '[]', set this field to: '• _No recent Form 4 insider trading actions filed within this lookback window._'\n\n"
...
async def get_insider_trading_activities_task(
        get_insider_activity_arg: GetInsiderActivityArg) -> InsiderTradingActivities:
    logger = get_run_logger()
    preferred_agent: PrefectAgent = _build_agent()
    user_prompt = \
        f"""
            f"Extract and summarize all SEC Form 4 insider transaction history for ticker: {get_insider_activity_arg.ticker}. "
    f"Focus on filings from the last {get_insider_activity_arg.recent_days} days. "
    "Populate the 'transactions' list and generate the 'slack_insider_activities' summary based on the results."
...
async def test_flow():
    ...
    logger.info("Workflow pipeline completed. Final Slack text generation payload len: %d",
