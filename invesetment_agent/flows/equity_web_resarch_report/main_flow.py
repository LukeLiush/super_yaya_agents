...
from web_scraping_task import HtmlSearchResult, web_scrapping_task
from web_search_task import SearchResult, web_search_task
...
async def build_flow(query: str):
    ...
    logger.info(
        "Successfully completed web search. Total unique items found: %d (filtered out %d duplicates)",
        len(unique_results),
    ...
    logger.info(
        "Successfully completed web scraping. Total items found: %d across %d queries",
        len(scraped_results),
    ...
    logger.info("Total HTML bytes scraped: %d bytes (%.2f KB)", total_html_bytes, total_html_bytes / 1024)
...
if __name__ == "__main__":
    ...
    #     build_flow(
    #         query=(
    #             "what is the price across amazon, walmart, target for corsair ddr4 3200mhz 16gb ram? "
    #             "and what is the price trend for the last 3 months?"
    #         )
    #     )
    # )
