...
container[UnitOfWork] = Singleton(
    SqliteUnitOfWork(
...
container[MarketDataProvider] = Singleton(YFinanceMarketDataAdapter(retrying=container[Retrying], windows=WINDOWS))
...
container[ReportSummarizer] = Singleton(
...
container[ReportNotifier] = Singleton(
    SlackReportNotifier(
...
container[CreateReportRequestUseCase] = Singleton(
    lambda: CreateReportRequestUseCase(container[SplitProvider], container[UnitOfWork])
)
container[GeneratePriceReportUseCase] = Singleton(
    lambda: GeneratePriceReportUseCase(container[MarketDataProvider], container[UnitOfWork], WINDOWS)
)
container[GenerateNewsReportUseCase] = Singleton(
    lambda: GenerateNewsReportUseCase(
        container[NewsProvider],
        container[UnitOfWork],
    )
)
container[GenerateInsiderReportUseCase] = Singleton(
    lambda: GenerateInsiderReportUseCase(container[InsiderProvider], container[UnitOfWork], recent_days=90)
)
...
if settings.inngest_enabled:
    container[ASGIAdapter] = Singleton(
        lambda: InngestASGI(
